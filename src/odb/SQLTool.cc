...
void SQLTool::run()
{
	...
	else
	{
		...
		if (inputFile_.size() == 0)
			p.parseFile(fileName, static_cast<DataHandle*>(0), odb::sql::SQLSelectFactory::instance().config());
		else
		{
			FileHandle fh(inputFile_);
			fh.openForRead();
			p.parseFile(fileName, &fh, odb::sql::SQLSelectFactory::instance().config());
		}
	}
}
