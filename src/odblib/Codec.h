...
public:
	...
	void dataHandle(void *p) { CodecInt8Missing<BYTEORDER>::dataHandle(static_cast<DataHandle*>(p)); }
...
public:
	...
	virtual void dataHandle(void *p) { intCodec.ds().dataHandle(static_cast<DataHandle*>(p)); }

	void gatherStats(double v) { CodecChars<BYTEORDER>::gatherStats(v); }
	void load(DataHandle *dh) { CodecChars<BYTEORDER>::load(dh); }
	void save(DataHandle *dh) { CodecChars<BYTEORDER>::save(dh); }
...
public:
	...
	void dataHandle(void *p) { intCodec.ds().dataHandle(static_cast<DataHandle*>(p)); }
...
template<typename BYTEORDER>
void CodecChars<BYTEORDER>::load(DataHandle *dh)
{
	Codec::loadBasics<BYTEORDER>(dh);
...
template<typename BYTEORDER>
void CodecChars<BYTEORDER>::save(DataHandle *dh)
{
	Codec::saveBasics<BYTEORDER>(dh);
	DataStream<BYTEORDER> ds(dh);
	hashTable_->save(ds);
