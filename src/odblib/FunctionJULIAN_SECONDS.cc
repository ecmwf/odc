...
FunctionJULIAN_SECONDS::FunctionJULIAN_SECONDS(const string& name, const expression::Expressions& args)
...
double FunctionJULIAN_SECONDS::eval(bool& missing) const
{
    ...
    return eclib::Date(year, month, day).julian() * 24 * 60 * 60 + hour * 3600 + min * 60 + sec;
}
