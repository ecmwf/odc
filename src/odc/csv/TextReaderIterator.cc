...
eckit::sql::BitfieldDef TextReaderIterator::parseBitfields(const std::string& c) {
    ...
    for (size_t i = 0; i < bs.size(); ++i) {
        ...
        // LOG_DEBUG_LIB(LibOdc) << "TextReaderIterator::parseBitfields:   bs[" << i << "] = " << bs[i] << " " << v << "
        // :  " << v.size() << std::endl;
...
void TextReaderIterator::parseHeader() {
    ...
    for (size_t i = 0; i < columns.size(); ++i) {
        LOG_DEBUG_LIB(LibOdc) << "TextReaderIterator::parseHeader: column " << i << " '" << columns[i] << "'"
                              << std::endl;
        ...
        const std::string columnType(
            S::upper(S::join(":", std::vector<std::string>(column.begin() + 1, column.end()))));

        if (!S::startsWith(columnType, "BITFIELD")) {
            LOG_DEBUG_LIB(LibOdc) << "TextReaderIterator::parseHeader: adding column " << columns_.size() << " '"
                                  << columnName << "' : " << columnType << std::endl;
            columns_.addColumn(columnName, columnType);
        }
        else {
            LOG_DEBUG_LIB(LibOdc) << "TextReaderIterator::parseHeader: adding BITFIELD " << columns_.size() << " '"
                                  << columns[i] << std::endl;
...
bool TextReaderIterator::isNewDataset() {
    return newDataset_;
}
