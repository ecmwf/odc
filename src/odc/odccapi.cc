...
unsigned int odb_get_headerBufferSize() {
    return ODBAPISettings::instance().headerBufferSize();
}
void odb_set_headerBufferSize(unsigned int n) {
    ODBAPISettings::instance().headerBufferSize(n);
}

unsigned int odb_get_setvbufferSize() {
    return ODBAPISettings::instance().setvbufferSize();
}
void odb_set_setvbufferSize(unsigned int n) {
    ODBAPISettings::instance().setvbufferSize(n);
}

unsigned int odc_format_version_major() {
    return odc::ODBAPIVersion::formatVersionMajor();
}
unsigned int odc_format_version_minor() {
    return odc::ODBAPIVersion::formatVersionMinor();
}
...
oda_select_iterator_ptr odb_create_select_iterator_from_file(oda_ptr co, const char* sql, const char* filename,
                                                             int* err) {
    Select* o(reinterpret_cast<Select*>(co));
...
int odb_write_iterator_set_bitfield(oda_write_iterator_ptr wi, int index, int type, const char* name,
                                    const char* bitfieldNames, const char* bitfieldSizes) {
...
int odb_read_iterator_get_missing_value(oda_read_iterator_ptr ri, int index, double* value) {
    ...
    if (index < 0 || long(r->columns().size()) < index) {
        std::stringstream ss;
        ss << "odb_read_iterator_get_missing_value: index " << index << " out of range, should be between 0 and "
           << r->columns().size();
...
int odb_read_iterator_get_bitfield(oda_read_iterator_ptr it, int index, char** bitfield_names, char** bitfield_sizes,
                                   int* nSize, int* sSize) {
    return get_bitfield<oda_read_iterator_ptr, ReaderIterator>(it, index, bitfield_names, bitfield_sizes, nSize, sSize);
}

int odb_select_iterator_get_bitfield(oda_select_iterator_ptr it, int index, char** bitfield_names,
                                     char** bitfield_sizes, int* nSize, int* sSize) {
    return get_bitfield<oda_select_iterator_ptr, SelectIterator>(it, index, bitfield_names, bitfield_sizes, nSize,
                                                                 sSize);
}
...
int odb_select_iterator_get_missing_value(oda_select_iterator_ptr ri, int index, double* value) {
    ...
    if (index < 0 || long(r->columns().size()) < index) {
        std::stringstream ss;
        ss << "odb_select_iterator_get_missing_value: index " << index << " out of range, should be between 0 and "
           << r->columns().size();
        throw UserError(ss.str());
    }
    *value = r->columns()[index]->missingValue();
