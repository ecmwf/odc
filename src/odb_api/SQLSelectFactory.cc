    ...
    std::pair<Expressions,std::vector<bool> > order_by)
{
	...
	if (from.size() == 0) resolveImplicitFrom(SQLSession::current(), from);
    ...
    //{
	//	reshift(select, minColumnShift_);
    //    if (where) where = reshift(where, minColumnShift_);
    //    reshift(order_by.first, minColumnShift_);
    //}
    SQLOutput *out (createOutput(SQLSession::current(), into, order_by.first.size()));

    if(distinct)              { out = new SQLDistinctOutput(out); }
    if(order_by.first.size()) { out = new SQLOrderOutput(out, order_by); }

	SQLSelect* r (new SQLSelect(select_list, from, where, order_by, out, config_, all, minColumnShift_));
